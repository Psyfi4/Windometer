...
import {
  C, FAMILY_COLOUR, TimeSeries, DensityScatter, IntervalPlot, BarsH, GroupedBars,
  MultiLine, FitScatter, BlandAltman, SignedBars, HistogramCurve, BandChart,
  PMatrix, LossCurves, WeightCurve,
      ...
      <div className="grid-15">
        <div>
          <BarsH
            labelWidth={230}
            rows={[
              { label: 'Linear interpolation (<6 h)', value: r.filledInterpolation, colour: C.teal, display: fmtInt(r.filledInterpolation) },
              { label: 'Monthly median (longer)', value: r.filledMonthlyMedian, colour: C.steel, display: fmtInt(r.filledMonthlyMedian) },
              { label: 'Record median (empty months)', value: r.filledGlobalMedian, colour: C.amber, display: fmtInt(r.filledGlobalMedian) },
            ]}
            xLabel="Hours filled"
            height={150}
          />
    ...
    <>
      ...
      <GroupedBars
        ...
        groups={HUB_HEIGHTS.map((h, i) => ({
          label: `${h} m`, colour: [C.teal, C.steel, C.amber][i],
      ...
      <div className="grid2" style={{ marginTop: '1.4rem' }}>
        <div>
          ...
          <MultiLine
            ...
            series={[
              { label: '10 m observed', colour: C.ink, values: monthly.map((m) => m.mwsObserved) },
              ...HUB_HEIGHTS.map((h, i) => ({
                label: `${h} m`, colour: [C.teal, C.steel, C.amber][i],
        ...
        <div>
          ...
          <MultiLine
            ...
            yLabel2="Scale s (m/s)"
            series={[
              { label: 'Shape k', colour: C.amber, values: monthly.map((m) => m.k) },
              { label: 'Scale s', colour: C.teal, axis2: true, values: monthly.map((m) => m.s) },
            ]}
          />
        </div>
      </div>
        ...
        <div className="grid2">
          <div>
            <FitScatter x={mlPairs.map((p) => p.actual)} y={mlPairs.map((p) => p.predicted)}
              colour={C.teal} label={`${bestModel} predicted`} fit={mlFit} />
          ...
          <div>
            <FitScatter x={qq.map((q) => q.observed)} y={qq.map((q) => q.theoretical)}
              colour={C.amber} label="Weibull quantile" fit={wbFit} />
            <div className="caption">
