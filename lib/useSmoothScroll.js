 ...
 *
 * One wheel detent travels a sixteenth of the viewport, so sixteen take you a
 * full page and a stage boundary lands exactly on a detent.
 *
...
export function useSmoothScroll(ref, {
  enabled = true,
  smoothTime = 0.035,   // spring response; an eighth-page hop lands in ~0.12 s
                        // across 7 frames. Below about 0.025 a hop spans
                        // fewer than 5 frames and stops reading as movement
                        // at all — the eye gets a start pose and an end pose.
                        // Speed past this point comes from pageFraction, not
                        // from shortening the animation further.
  pageFraction = 0.0625,// a full 100-unit detent travels a sixteenth of the
                        // viewport — about half what a browser moves natively
  maxEventPages = 1.4,  // ceiling on what one event can ask for
  maxVelocity = 20000,  // px per second ceiling, kept above the peak the
                        // spring actually reaches so it never clips. A full
                        // page in 0.17 s tops out near 17300, so a lower
                        // ceiling would quietly cap the speed and undo the
                        // very thing it was raised for.
  snapSelector = '.stage',
  snapQuietMs = 110,    // wheel silence before settling onto a section
  snapMaxPull = 0.035,  // settle only within this fraction of the viewport.
                        // At 0.40 a pause part-way through a notch dragged you
                        // back nearly half a page, against the direction you
                        // were scrolling. With a full page per detent the
                        // scroll already lands on stage boundaries, so this
                        // only needs to tidy small residuals.
} = {}) {
  useEffect(() => {
    ...
    /**
     * The loop owns the scroll position while it runs, so anything else that
     * also moves it has to be off. CSS scroll-snap pulls toward a snap point
     * while the spring eases toward its own target and the two oscillate
     * against each other; scroll-behavior would animate programmatic scrolls
     * on top of the loop.
     *
     * Set here rather than left to the stylesheet. The rule that did this
     * lives in globals.css, and a hook whose correctness depends on a
     * particular CSS file having been deployed alongside it is a hook that
     ...
     *
     * Detected by position, not by a flag. Scroll events are dispatched
     * asynchronously: a flag set around the `el.scrollTop = current` write is
     * already cleared by the time the resulting event arrives, so every frame
     * the spring wrote came back looking like an external scroll and reset the
     * target. The animation died and restarted continuously, which read as the
     * page juddering back and forth.
     *
     * Comparing against where we last put it has no timing assumption in it.
     */
    const onScroll = () => {
      if (Math.abs(el.scrollTop - current) < 2) return;   // our own write
      target = el.scrollTop;
