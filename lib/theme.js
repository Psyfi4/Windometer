...
/**
 * Three adjacent stops centred on one, for a station to breathe between.
 * Adjacent rather than scattered, so the cycle stays inside its own region of
 * the ramp and the station keeps a recognisable colour throughout.
 */
export function band(i) {
  const n = SPECTRUM.length;
  // slide the window inward at the ends rather than clamping, or a station on
  // the first or last stop would breathe between a colour and itself
  const start = Math.max(0, Math.min(n - 3, i - 1));
  const win = [SPECTRUM[start], SPECTRUM[start + 1], SPECTRUM[start + 2]];
  return [SPECTRUM[i], ...win.filter((c) => c !== SPECTRUM[i])];
}
