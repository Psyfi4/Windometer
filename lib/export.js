...
/**
 * Force every stage to lay out, run something, then put it back.
 *
 * The stages carry content-visibility: auto, so one you have never scrolled to
 * has no layout at all — its charts are in the DOM but measure zero, and any
 * size filter discards them. Exporting would then quietly save only the
 * sections you happened to have looked at, which is the worst kind of wrong:
 * plausible, and silent.
 *
 * Two frames are awaited rather than one. The first lets the style change take
 * effect; the second lets layout settle for the subtrees that were skipped,
 * which is when the charts finally have a size to measure.
 */
export async function withAllStagesRendered(fn) {
...
/**
 * Every chart on the page, with a name taken from the page.
 *
 ...
 *
 * Pass `section` to take only one part of the page — matched loosely against
 * the stage title, so 'comparison' finds the Comparison stage.
 */
export function collectCharts(root = document, { section = null } = {}) {
  const want = section ? String(section).toLowerCase() : null;

  const svgs = [...root.querySelectorAll('.main svg')]
    .filter((s) => !s.closest('.hero-mark'))
    .filter((s) => {
      if (!want) return true;
      const title = s.closest('.stage')?.querySelector('.stage-title')?.textContent ?? '';
      return title.toLowerCase().includes(want);
    })
    .filter((s) => {
